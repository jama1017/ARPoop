...
public class ScreenShoter {
    ...
    public static void CaptureScreenShot(string name, bool saveInGallery = true) {
        ...
        if (saveInGallery) {
            ...
            var bitmap = bitmapfactory.CallStatic<AndroidJavaObject>("decodeByteArray", bytes, 0, bytes.Length);
            // with bitmap, save it to gallery
            var outputfile = new AndroidJavaObject("java.io.File", AndroidGalleryPath, filename);
            var outputstream = new AndroidJavaObject("java.io.FileOutputStream", outputfile);
